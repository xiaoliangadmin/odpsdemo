...
public class Main {
    ...
    public static void main(String[] args) {
        File file = new File("D:\\FUDIAN\\Desktop\\ttt.xls");

        try {
            InputStream is = new FileInputStream(file.getAbsolutePath());
            Workbook workbook = Workbook.getWorkbook(is);
            Sheet sheet = workbook.getSheet(0);//创建集成等任务配置信息
            Sheet sheet2 = workbook.getSheet(1);//创建sql等任务配置信息
            ...
            for (int i=1; i<sheet.getRows(); i++) {
                ...
                if (dataModel.getFileType() == 99) { //创建虚拟节点
                    ...
                    flag = service.createVINode(dataModel);
                    if (flag) {
                        messageInfo = "虚拟任务节点 " + dataModel.getFileName() + " 创建并提交成功！";
                    } else {
                        messageInfo = "虚拟任务节点 " + dataModel.getFileName() + " 创建或提交失败！";
                    }
                } else if (dataModel.getFileType() == 6) { //创建shell节点
                    Service service = new Service();
                    flag = service.createShellNode(dataModel);
                    if (flag) {
                        messageInfo = "shell任务节点 " + dataModel.getFileName() + " 创建并提交成功！";
                    } else {
                        messageInfo = "shell任务节点 " + dataModel.getFileName() + " 创建或提交失败！";
                    }
                    ...
                    Service service = new Service();
                    flag = service.createDINode(dataModel, sheet3);
                    if (flag) {
                        messageInfo = "数据集成节点 " + dataModel.getFileName() + " 创建并提交成功！";
                    } else {
                        messageInfo = "数据集成节点 " + dataModel.getFileName() + " 创建或提交失败！";
                    }
                } else {
            ...
            if (z_flag) {
                for (int i=1; i<sheet2.getRows(); i++) {
                    ...
                    if (dataModel.getFileType() == 99) { //创建虚拟节点
                        ...
                        Service service = new Service();
                        flag = service.createSqlNode(dataModel);
                        if (flag) {
                            messageInfo = "odps sql任务节点 " + dataModel.getFileName() + " 创建并提交成功！";
                        } else {
                            messageInfo = "odps sql任务节点 " + dataModel.getFileName() + " 创建或提交失败！";
                        }
                    }else {
                        System.out.println("节点类型配置错误");
                    }
